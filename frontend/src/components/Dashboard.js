...
// In your Dashboard component, update the location tracking part:
useEffect(() => {
  ...
  LocationService.startTracking((location) => {
    ...
    if (location && typeof location.latitude === 'number' && typeof location.longitude === 'number') {
      ...
      // Update location on server if there's an active trip
      if (activeTrip) {
    ...
    const startLocationTracking = async () => {
      try {
        ...
        LocationService.startTracking((location) => {
          ...
          // Send location updates to backend if there's an active trip
